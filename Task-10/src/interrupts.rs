...
extern "x86-interrupt" fn keyboard_interrupt_handler(_stack_frame: InterruptStackFrame) {
    ...
    let scancode_set1 = ScancodeSet1::default();

    lazy_static! {
        static ref KEYBOARD: Mutex<Keyboard<layouts::Us104Key, ScancodeSet1>> = Mutex::new(
            Keyboard::new(layouts::Us104Key, Scancode_set1, HandleControl::Ignore)
        );
    }
            ...
            DecodedKey::Unicode(character) => {
                ...
                unsafe{
                    if success(&CHARS) {
                        ...
                        println!("{}",ASCII_AMFOSS_TOP);
                        println!("{}",port_address);
                        println!("{}",ASCII_AMFOSS_MID);
                        print_array(&CHARS);
                        println!("{}",ASCII_AMFOSS_BOTTOM);
